...
let ResourceController = class ResourceController {
    ...
    async create(data, files) {
        if (data?.resourceTypeId !== undefined)
            data.resourceTypeId = Number(data.resourceTypeId);
        if (files?.icon?.[0])
            data.icon = `/uploads/images/${files.icon[0].filename}`;
...
__decorate([
    (0, common_1.Post)(),
    (0, common_1.UseInterceptors)((0, platform_express_1.FileFieldsInterceptor)([{ name: 'icon', maxCount: 1 }], {
